...
import { GasRecord, AxisConfig, PentagonConfig } from "../config";
...
export class UIController {
    ...
    constructor(private root: HTMLElement, zoomConfig: { min: number, max: number, default: number }) {
        ...
        ["P1", "P2"].forEach(type => {
            ...
            Object.assign(btn.style, {
                ...
                background: this.state.activeShape === type ? "#0078d4" : "#f4f4f4",
                color: this.state.activeShape === type ? "#fff" : "#333",
                fontWeight: "bold", borderRadius: "4px", fontSize: "14px",
                transition: "background 0.2s"
            });
            btn.onclick = () => {
                this.state.activeShape = type;
    ...
    private updateToggleButtons(): void {
        Array.from(this.toggleContainer.children).forEach((child, i) => {
            ...
            const t = ["P1", "P2"][i];
            el.style.background = this.state.activeShape === t ? "#0078d4" : "#f4f4f4";
            el.style.color = this.state.activeShape === t ? "#fff" : "#333";
        });
    }
